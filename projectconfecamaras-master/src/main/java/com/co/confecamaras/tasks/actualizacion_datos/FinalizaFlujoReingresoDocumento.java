...
import static com.co.confecamaras.userinterfaces.actualizacion_datos.ActualizacionMutacionCajaPage.OPCION_REPATO_Y_ESCANEO;
import static net.serenitybdd.screenplay.Tasks.instrumented;
import static net.serenitybdd.screenplay.matchers.WebElementStateMatchers.isPresent;

public class FinalizaFlujoReingresoDocumento implements Task {
    @Override
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                ...
                WaitUntil.the(BOTON_CERRAR_ACCIONES, isPresent()).forNoMoreThan(120).seconds(),
                Click.on(BOTON_CERRAR_ACCIONES),
                Click.on(BOTON_MENU_CAMARA_DE_COMERCIO),
                WaitUntil.the(BOTON_BANDEJAS, isPresent()).forNoMoreThan(120).seconds(),
                Click.on(BOTON_BANDEJAS),
                WaitUntil.the(OPCION_REGISTROS_PUBLICOS, isPresent()).forNoMoreThan(120).seconds(),
                Click.on(OPCION_REGISTROS_PUBLICOS),
                WaitUntil.the(OPCION_REPATO_Y_ESCANEO, isPresent()).forNoMoreThan(120).seconds(),
                ...
                WaitUntil.the(LISTA_USUARIO_SELECCION, isPresent()).forNoMoreThan(120).seconds(),
                ...
                WaitUntil.the(OPCION_ESTUDIO_REGMTIL, isPresent()).forNoMoreThan(120).seconds(),
                Click.on(OPCION_ESTUDIO_REGMTIL),
                WaitInterrupted3Segundos.esperaConstante3(),
                SwitchToNewWindow.switchToNewTab(),
                WaitUntil.the(CAMPO_BUSCAR_REGMTIL, isPresent()).forNoMoreThan(120).seconds(),
                Click.on(CAMPO_BUSCAR_REGMTIL),
                Enter.theValue(codigo).into(CAMPO_BUSCAR_REGMTIL),
                Hit.the(Keys.TAB).into(CAMPO_INGRESO_COD_BARRAS_INICIAL),
                WaitInterrupted3Segundos.esperaConstante3(),
                WaitUntil.the(BOTON_DEVOLVER_REQUERIMIENTO, isPresent()).forNoMoreThan(120).seconds(),
                ...
                WaitUntil.the(BOTON_APLICAR_FORMULARIO_, isPresent()).forNoMoreThan(120).seconds(),
                Scroll.to(BOTON_APLICAR_FORMULARIO_).andAlignToTop(),
                Click.on(BOTON_APLICAR_FORMULARIO_),
                WaitUntil.the(BOTON_CONTINUAR_ESCANEO_DIGITALIZACION, isPresent()).forNoMoreThan(120).seconds(),
                Click.on(BOTON_CONTINUAR_ESCANEO_DIGITALIZACION),
                SwitchToNewWindow.switchToNewTab(),
                CloseSecondTab.now(),
                ...
                CloseSecondTab.now(),
                WaitInterrupted2Segundos.esperaConstante2(),
                WaitUntil.the(BOTON_BUSCAR_CRTL_K, isPresent()).forNoMoreThan(120).seconds(),
                Click.on(BOTON_BUSCAR_CRTL_K),
                WaitUntil.the(CAMPO_BUSCAR_CRTL_K, isPresent()).forNoMoreThan(120).seconds(),
                Enter.theValue("Ruta de documentos").into(CAMPO_BUSCAR_CRTL_K),
                Click.on(OPCION_RUTA_DOCUMENTOS),
                WaitUntil.the(CAMPO_INGRESO_VALOR_BUSQUEDA, isPresent()).forNoMoreThan(120).seconds(),
                Enter.theValue(codigo).into(CAMPO_INGRESO_VALOR_BUSQUEDA),
                Click.on(BOTON_CONSULTAR_),
                WaitUntil.the(BOTON_ENTREGAR_AL_USUARIO, isPresent()).forNoMoreThan(120).seconds(),
                Click.on(BOTON_ENTREGAR_AL_USUARIO),
                WaitUntil.the(BOTON_SI_INFORMATIVO, isPresent()).forNoMoreThan(120).seconds(),
                Click.on(BOTON_SI_INFORMATIVO),
                WaitUntil.the(BOTON_OK_SI_INFORMATIVO, isPresent()).forNoMoreThan(120).seconds(),
    ...
    public static FinalizaFlujoReingresoDocumento Finaliza(){
        return instrumented(FinalizaFlujoReingresoDocumento.class);
    }
}
