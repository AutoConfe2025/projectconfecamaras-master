...
import static com.co.confecamaras.userinterfaces.Certificados.UsuarioPublico.CoCertificadoOcPjUsPu.*;

public class CoCertificadoOcPjUsPu implements Task {

    public final Map<String, String> data;

    public CoCertificadoOcPjUsPu(Map<String, String> data) {
        this.data = data;
    }
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                WaitInteractions.untilAppears(BTN_CONSULTAR_PJ),
                WaitInterrupted3Segundos.esperaConstante3(),
                Click.on(BTN_CONSULTAR_PJ),
                WaitInteractions.untilBeEnable(BTN_CERTIFICADO_AUTOMATICO_IMEDIATO),
                Click.on(BTN_CERTIFICADO_AUTOMATICO_IMEDIATO),
                WaitInteractions.untilBeEnable(TXT_CERTIFICADO_PJ),
                Enter.theValue(data.get("Certificados")).into(TXT_CERTIFICADO_PJ),
                Click.on(BTN_CONTINUAR_SOLICITUD_CERTIFIACDO_PJ),
                ModalLoader.modalLoader(),
    ...
    public static CoCertificadoOcPjUsPu coCertificadoOcPjUsPu(Map<String, String> data) {
        return Tasks.instrumented(CoCertificadoOcPjUsPu.class, data);
    }
}
