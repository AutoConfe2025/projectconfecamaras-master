...
public class IngresoPorCuenta implements Task {
    @Override
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                Enter.theValue(BUSQUEDA_INGRESO_CUENTAS).into(TXT_BUSQUEDA_INPUT),
                WaitInteractions.untilBeEnable(BTN_INGRESO_CUENTA),
                Click.on(BTN_INGRESO_CUENTA)
        );

        actor.attemptsTo(
                CambiarALaSiguientePestana.siguiente(),
                WaitInteractions.untilBeEnable(INPUT_CUENTA_INICIAL),
                Enter.theValue(VALOR_CUENTA_INICIAL).into(INPUT_CUENTA_INICIAL),
                WaitInteractions.untilBeEnable(INNPUT_CUENTA_FINAL),
                Enter.theValue(VALOR_CUENTA_FINAL).into(INNPUT_CUENTA_FINAL),
        ...
        actor.attemptsTo(
                WaitInteractions.untilBeEnable(GENERAR_EXCEL),
                Click.on(GENERAR_EXCEL)

        );
    }

    public static IngresoPorCuenta consultar() {
        return Tasks.instrumented(IngresoPorCuenta.class);
    }
}
