...
public class DoSolicitudCertificadosPNM implements Task {
    ...
    public DoSolicitudCertificadosPNM(String consulta, String numero, String numCert, String tipoIdent, String identificacion, String correo) {
    ...
    public static Performable solicitudCertificadosPNM(String consulta, String numero, String numCert, String identificacion, String correo, String tipoIdent){
        return instrumented(DoSolicitudCertificadosPNM.class, consulta, numero, numCert, identificacion, correo, tipoIdent);
    }
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                WaitInterrupted2Segundos.esperaConstante2(),
                Clear.field(SolicitudCertificadosUI.CERTIFICADO_NUM_INPUT).then(Enter.theValue(numCert).into(SolicitudCertificadosUI.CERTIFICADO_NUM_INPUT)),
                JavaScriptClick.on(SolicitudCertificadosUI.CONTINUAR_BTN),
                WaitConstant.esperaConstante(),
                Switch.toNewWindow(),
                Clear.field(RecibirPago.IDENTIFICACION_INPUT).then(Enter.theValue(identificacion).into(RecibirPago.IDENTIFICACION_INPUT)),
                JavaScriptClick.on(RecibirPago.VERIFICAR_IDENT_BTN),
                Enter.theValue(correo).into(RecibirPago.EMAIL_CONFIRMACION_INPUT),
                ...
                Ensure.that(VALOR_TOTAL).text().isEqualTo("$5,800.00"),
                Ensure.that(VALOR_PAGADO).text().isEqualTo("$5,800.00"),
                Ensure.that(VALOR_VUELTAS).text().isEqualTo("$0.00"),
                /////////////////
