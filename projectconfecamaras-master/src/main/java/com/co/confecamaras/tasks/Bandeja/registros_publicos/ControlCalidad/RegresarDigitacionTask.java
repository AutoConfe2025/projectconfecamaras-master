...
public class RegresarDigitacionTask implements Task {
    @Override
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                Click.on(AccionesPage.LINK_ACCION.of("Regresar a Digitacion")),
                WaitSeconds.seconds(1),
                GetTextOfElement.de(ControlCalidadPage.MSG_POUP),
                Click.on(AccionesPage.BTN_CANCELAR_F)
        );
    }
    public static RegresarDigitacionTask digitacion(){
        return new RegresarDigitacionTask();
    }
}
