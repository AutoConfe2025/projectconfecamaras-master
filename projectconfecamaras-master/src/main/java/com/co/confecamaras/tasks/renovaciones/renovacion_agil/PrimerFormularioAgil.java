...
import static com.co.confecamaras.userinterfaces.renovaciones.FlujoAgilEsadlPage.CAMPO_INGRESO_VALOR_ACTIVOS_VINCULADOS;
...
public class PrimerFormularioAgil implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        String valor = actor.recall("valorActivos");
        actor.attemptsTo(
        ...
        if (CAMPO_INGRESO_VALOR_ACTIVOS_VINCULADOS.resolveFor(actor).isPresent()) {
            actor.attemptsTo(
        ...
        if (CAMPO_FECHA_CIIU2.resolveFor(actor).isPresent()) {
            actor.attemptsTo(
        ...
        if (CAMPO_ACTIVO_TOTAL.resolveFor(actor).isPresent()) {
