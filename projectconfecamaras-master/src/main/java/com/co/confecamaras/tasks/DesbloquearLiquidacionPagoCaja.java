...
public class DesbloquearLiquidacionPagoCaja implements Task {

    private String idNumeroRecuperacion;

    public DesbloquearLiquidacionPagoCaja(String idNumeroRecuperacion) {
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                WaitInteractions.untilAppears(BTN_DESBLOQUEAR_LIQUIDACION_PAGO_CAJA),
                Scroll.to(BTN_DESBLOQUEAR_LIQUIDACION_PAGO_CAJA).andAlignToBottom(),
                JavaScriptClick.on(BTN_DESBLOQUEAR_LIQUIDACION_PAGO_CAJA),
                WaitInterrupted2Segundos.esperaConstante2(),
                WaitInteractions.untilBeEnable(TXT_NUMERO_RECUPERACION_DESBLOQUEAR_LIQUIDACION),
                ...
                JavaScriptClick.on(BTN_CONTINUAR_DESBLOQUEAR_LIQUIDACION)
        );
    }

    public static DesbloquearLiquidacionPagoCaja enConfecamaraSII(String idNumeroRecuperacion) {
        return instrumented(DesbloquearLiquidacionPagoCaja.class, idNumeroRecuperacion);
    }
}