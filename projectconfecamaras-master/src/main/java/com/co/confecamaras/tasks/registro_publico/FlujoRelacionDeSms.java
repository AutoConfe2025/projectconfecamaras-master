...
public class FlujoRelacionDeSms implements Task {
    @Override
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                Click.on(CAMPO_FECHA_INICIAL_DATA_PICKER),
                Click.on(CAMPO_FECHA_FINAL_DATA_PICKER),
                Enter.theValue("2026-01-15").into(CAMPO_FECHA_FINAL_DATA_PICKER),
                Click.on(CAMPO_FECHA_FINAL_DATA_PICKER),
    ...
    public static FlujoRelacionDeSms RelacionSMS(){
        return new FlujoRelacionDeSms();
    }
}
