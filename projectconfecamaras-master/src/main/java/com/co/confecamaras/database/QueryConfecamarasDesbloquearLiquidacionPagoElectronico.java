...
public class QueryConfecamarasDesbloquearLiquidacionPagoElectronico {

    public Map<String, Object> buscarData() throws SQLException {
        Connection con = getCon();
        Statement smtDoc= con.createStatement();
        String queryDoc = "SELECT numerorecuperacion\n" +
                "FROM sii_manizales.mreg_liquidacion \n" +
                "WHERE numerorecuperacion\n" +
                "AND idestado = '06'\n" +
                "LIMIT 1";
        ...
        while (rsDoc.next()) {
            String key = rsDoc.getString("numerorecuperacion");
